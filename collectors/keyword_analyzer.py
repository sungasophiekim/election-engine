...
if __name__ == "__main__":
    ...
    print(f"=== '{result.keyword}' 상세 분석 ===")
    print(f"분석 건수: {result.total_analyzed}건\n")

    print("연관 단어 TOP 10:")
    for w in result.co_words[:10]:
        print(f"  {w['word']:10s} ({w['count']}회)")

    print(f"\n감정 톤: {result.dominant_tone} (스코어 {result.tone_score:+.2f})")
    for tone, count in sorted(result.tone_distribution.items(), key=lambda x: -x[1]):
        bar = "█" * min(count, 30)
        print(f"  {tone:4s} {count:>3}회 {bar}")

    print(f"\n프레임:")
    for n in result.key_narratives[:5]:
        print(f"  [{n['count']:>2}회] {n['narrative']}: {n['sample'][:50]}")

    print(f"\n주체: {result.about_whom}")
    print(f"채널: {result.who_talks}")

    print(f"\n뉴스 샘플:")
    for t in result.news_samples[:3]:
        print(f"  · {t[:60]}")
