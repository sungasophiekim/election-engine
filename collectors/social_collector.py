...
if __name__ == "__main__":
    ...
    print("=== 후보 소셜 버즈 비교 ===")
    print("=" * 60)
    buzz = compare_candidate_buzz("박완수", ["김경수", "전희영"])
    for name in ["박완수", "김경수", "전희영"]:
        b = buzz[name]
        print(f"\n  {name}:")
        print(f"    블로그: {b['blog']:,}건 | 카페: {b['cafe']:,}건 | 동영상: {b['video']:,}건")
        print(f"    총 언급: {b['total']:,}건 | 감성: {b['sentiment']:+.3f}")
    print(f"\n  >>> 버즈 리더: {buzz['buzz_leader']}")
    print(f"  >>> 감성 리더: {buzz['sentiment_leader']}")
    ...
    print(f"\n  총 언급: {summary['total_mentions']:,}건")
    print(f"  감성: 긍정 {summary['sentiment']['positive']:.3f} / "
          f"부정 {summary['sentiment']['negative']:.3f} / "
          f"순감성 {summary['sentiment']['net']:+.3f}")
    print(f"  가장 활발한 채널: {summary['hottest_channel']}")
    print(f"  가장 뜨거운 키워드: {summary['hottest_keyword']}")
    print(f"  우리 후보 버즈: {summary['candidate_buzz']:,}건")
    for opp, cnt in summary["opponent_buzz"].items():
        print(f"  상대 후보 [{opp}] 버즈: {cnt:,}건")

    for ch in ("blog", "cafe", "video"):
        ch_label = {"blog": "블로그", "cafe": "카페", "video": "동영상"}[ch]
        print(f"\n  [{ch_label}]")
        for sig in signals[ch]:
            print(f"    {sig.keyword}: 총 {sig.total_count:,}건 | "
                  f"24h {sig.recent_24h}건 | "
                  f"부정 {sig.negative_ratio:.1%} | "
                  f"긍정 {sig.positive_ratio:.1%} | "
                  f"참여도 {sig.engagement_score:.3f}")

    # 3. 인기 유튜브 영상
    print("\n" + "=" * 60)
    print("=== 인기 유튜브/동영상 ===")
    print("=" * 60)
    videos = get_youtube_top_videos("경남도지사 선거")
    for i, v in enumerate(videos, 1):
        play = f"{v['play_count']:,}" if v['play_count'] else "N/A"
        print(f"  {i}. [{play}회] {v['title']}")
        print(f"     {v['link']}")
