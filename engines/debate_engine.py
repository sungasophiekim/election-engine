...
if __name__ == "__main__":
    ...
    # 박완수 상대 토론 준비
    prep = engine.prepare("박완수")

    # 전체 보고서 출력
    report = engine.format_report(prep)
    print(report)
    ...
    print("  요약 통계")
    print(f"{'=' * 74}")
    print(f"  상대 후보: {prep.opponent}")
    print(f"  예상 질문: {len(prep.expected_questions)}개")
    ...
    print(f"    - 출처별: {', '.join(f'{k} {v}개' for k, v in source_counts.items())}")
    print(f"    - 난이도별: {', '.join(f'{k} {v}개' for k, v in diff_counts.items())}")
    print(f"  공격 스크립트: {len(prep.attack_scripts)}개")
    print(f"  방어 스크립트: {len(prep.defense_scripts)}개")
    print(f"  피벗 메시지: {len(prep.pivot_messages)}개")
    print(f"  레드라인: {len(prep.red_lines)}개")
    print(f"  바디랭귀지 팁: {len(prep.body_language_tips)}개")
    print(f"{'=' * 74}")
