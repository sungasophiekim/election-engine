...
if __name__ == "__main__":
    ...
    test_cases = [
        ContentDraft(
            ...
            text="홍길동 후보는 경남형 스마트산단 5개소 조성으로 일자리 3만 개를 창출하겠다고 밝혔다.",
            tenant_id="gyeongnam_2026",
        ),
        ContentDraft(
            content_type=ContentType.SNS_POST,
            text="내로남불 상대 후보와 달리 우리 후보는 진정한 일꾼입니다! 반드시 당선됩니다!",
            tenant_id="gyeongnam_2026",
        ),
    ]

    print("=" * 60)
    print("Message Consistency Validator — 테스트 결과")
    print("=" * 60)

    for draft in test_cases:
        # 금기어 하드 필터만 테스트 (API 키 없이)
        violations = _check_forbidden_words(draft.text, SAMPLE_GYEONGNAM_CONFIG)
        rule_score = _calculate_rule_based_score(violations, [], [])

        print(f"\n콘텐츠 유형: {draft.content_type.value}")
        print(f"텍스트: {draft.text[:60]}...")
        print(f"금기어 위반: {violations if violations else '없음'}")
        print(f"룰 기반 점수: {rule_score:.2f}")
        print(f"배포 가능: {'❌ 반려' if violations else '✅ 통과 (Claude 검증 필요)'}")
