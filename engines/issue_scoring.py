...
if __name__ == "__main__":
    ...
    results = score_multiple_signals(test_signals, SAMPLE_GYEONGNAM_CONFIG)

    print("=" * 60)
    print("Issue Scoring Engine — 테스트 결과")
    print("=" * 60)
    for r in results:
        level_emoji = {
            CrisisLevel.CRISIS: "🔴",
        ...
        print(f"\n{level_emoji} [{r.level.name}] {r.keyword}")
        print(f"   점수: {r.score} / 100")
        print(f"   반감기: {r.estimated_halflife_hours}h")
        print(f"   구성: {r.breakdown}")
