...
def classify_media_tier(source_url: str) -> int:
    """미디어 티어 분류: 1=방송/메이저, 2=인터넷뉴스/지역주요, 3=블로그/카페"""
    ...
    # 경남 지역 주요 언론 (Tier 2로 분류 — 지역에서는 Tier 1급 영향력)
    regional = ["knn.co.kr", "gnews.kr", "idomin.com", "gnnews.co.kr",
                "gnmaeil.com", "cwsm.kr", "mbc-gn.co.kr", "cj-gn.co.kr",
                "busan.com", "kookje.co.kr", "jinjunews.co.kr",
                "yangsanilbo.com", "newsgyeongnam.kr", "gnynews.co.kr",
                "gnsdaily.kr", "newssacheon.co.kr", "gctnews.com"]
...
def is_regional_media(source_url: str) -> bool:
    """경남 지역 언론사 여부."""
    regional_domains = ["knn.co.kr", "gnews.kr", "idomin.com", "gnnews.co.kr",
                        "gnmaeil.com", "cwsm.kr", "geojenews.co.kr", "gimhaenews.co.kr",
                        "mbc-gn.co.kr", "cj-gn.co.kr", "busan.com", "kookje.co.kr",
                        "jinjunews.co.kr", "yangsanilbo.com", "newsgyeongnam.kr",
                        "gnynews.co.kr", "gnsdaily.kr", "newssacheon.co.kr", "gctnews.com"]
    return any(d in source_url for d in regional_domains)
...
def _build_impact_hints(
    ...
    """
    v2: 기사 수준 임팩트 힌트 생성.
    Naver API 원시 데이터에서 추출 가능한 지표만 사용.

    Returns:
        {
            "tier1_count": int,        # 방송/메이저 기사 수
            "tier2_count": int,        # 인터넷뉴스 기사 수
            "tier3_count": int,        # 블로그/카페 기사 수
            "source_diversity": int,   # 고유 언론사 수 (많을수록 확산 중)
            "temporal_cluster": float, # 최근 6h 기사 비중 (1.0=모두 최근)
            "headline_neg_ratio": float, # 제목 only 부정 비율 (본문 제외)
            "net_sentiment": float,    # -1.0~1.0 (analyze_sentiment 결과)
            "defense_active": bool,    # 반박/해명 기사 존재 여부
        }
    """
    tier1_count = sum(1 for t in tiers if t == 1)
    tier2_count = sum(1 for t in tiers if t == 2)
    ...
    # 제목 전용 부정 비율 (description 제외 → 더 강한 시그널)
    headline_neg = 0
    for a in articles:
        if any(kw in a["title"] for kw in NEGATIVE_KEYWORDS):
            headline_neg += 1
    headline_neg_ratio = round(headline_neg / max(len(articles), 1), 3)

    # 방어 기사 존재 여부
    defense_active = any(
        any(p in a["title"] for p in DEFENSE_PATTERNS)
        for a in articles
    )

    # 시간 집중도: 최근 6h / 24h
    temporal_cluster = round(recent_6h / max(recent_24h, 1), 3)
    ...
    return {
        ...
        "headline_neg_ratio": headline_neg_ratio,
        "net_sentiment": sentiment.get("net_sentiment", 0.0),
...
ENDORSEMENT_KEYWORDS = [
    "지지선언", "공식 지지", "지지 선언", "추대", "단일화",
    "연대", "공동선대위", "합류", "지지 철회", "탈당",
    "지지 번복", "이탈",
]

def collect_endorsement_signals(
    candidate_name: str,
    opponents: list[str] = None,
    region: str = "",
) -> list[dict]:
    """
    v2: 조직/단체 지지·이탈 시그널 수집.
    뉴스 API에서 endorsement 키워드로 검색하여 추출.
    별도 모듈 대신 naver_news.py에 통합 (데이터 소스가 동일하므로).
        ...
        [
            {
                "headline": str,
                "target": str,          # 지지 대상 후보
                "org_hint": str,        # 조직/단체명 힌트 (제목에서 추출)
                "type": "endorse" | "withdraw" | "coalition",
                "source": str,
                "pub_date": str,
            },
            ...
    ...
    """
    opponents = opponents or []
    all_names = [candidate_name] + opponents
    results = []

    for name in all_names:
        for ek in ENDORSEMENT_KEYWORDS[:6]:  # API 호출 절약: 핵심 키워드만
            query = f"{name} {ek}"
            if region:
                query = f"{name} {region} {ek}"
            try:
            ...
            for art in articles:
                title = art["title"]
                # 이미 수집된 것과 중복 제거
                if any(r["headline"] == title for r in results):
                    continue

                # 유형 분류
                if any(w in title for w in ["지지 철회", "이탈", "번복", "탈당"]):
                    sig_type = "withdraw"
                elif any(w in title for w in ["단일화", "연대", "합류", "공동"]):
                    sig_type = "coalition"
                else:
                    sig_type = "endorse"
                ...
                results.append({
                    "headline": title,
                    "target": name,
                    "org_hint": _extract_org_hint(title, name),
                    "type": sig_type,
...
def _extract_org_hint(title: str, candidate_name: str) -> str:
    """제목에서 조직/단체명 힌트 추출 (간이)"""
    # 후보 이름과 endorsement 키워드를 제거한 나머지에서 추출
    cleaned = title
    for word in [candidate_name] + ENDORSEMENT_KEYWORDS:
        cleaned = cleaned.replace(word, "")
    # 따옴표 안의 내용이나 짧은 명사구 추출
...
def collect_issue_signals(
    ...
    v2: impact_hints, raw_articles 를 _last_collection_meta에 저장하여
        다운스트림에서 참조 가능.
    """
    signals = []
    _last_collection_meta.clear()

    for kw in keywords:
        try:
            # 3페이지 수집 (최대 300건) — 정확도 향상
            articles = search_news(kw, display=100, pages=3)
            # count_mentions() 제거 — 리턴값 어디서도 미사용, API 콜 절약
        ...
        # 포털 트렌딩 추정: 300건 샘플에서 200건 이상 (150→200 상향, 변별력 확보)
        portal_trending = recent_24h >= 200

        # TV 보도 여부: 방송사(티어1) 기사가 5건 이상 (3→5 상향)
        tier1_count = sum(1 for t in tiers if t == 1)
        tv_reported = tier1_count >= 5
        ...
        # v2: 임팩트 힌트 생성
        impact_hints = _build_impact_hints(
            articles, tiers, recent_24h, recent_6h, sentiment
        )

        # v2: 메타데이터 저장 (unified_collector에서 참조)
        _last_collection_meta[kw] = {
...
# v2: 마지막 수집의 메타데이터 (unified_collector에서 참조)
_last_collection_meta: dict = {}

def get_last_collection_meta() -> dict:
    """v2: 마지막 collect_issue_signals() 호출의 메타데이터 반환"""
    return _last_collection_meta
