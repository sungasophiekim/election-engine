...
def _auto_druking_residual() -> dict:
    """드루킹 잔여 — '드루킹/김경수 전과/수감' 뉴스 빈도 자동"""
    snap = _load_enrichment_snapshot()
    ii = snap.get("issue_indices", {})
    keywords = ["드루킹", "댓글조작", "전과", "수감"]
    found = []
    for kw in keywords:
        for k, v in ii.items():
            if kw in k:
                found.append(v.get("index", 0))
    if not found:
        # 뉴스에 안 나오면 반감기 적용 기본값
        return {"name": "드루킹 잔여 효과", "value": -1.2, "confidence": "medium",
                "reason": "반감기 18~24개월, 잔여 40~50% (기본값)"}
    avg = sum(found) / len(found)
    # 이슈 활성도 높으면 (재점화) → 더 부정적
    value = round(max(-3.0, min(-0.3, -0.5 - avg * 0.03)), 1)
    return {"name": "드루킹 잔여 효과", "value": value, "confidence": "medium",
            "reason": f"드루킹 이슈 활성도 {avg:.0f} → 자동산출 {value:+.1f}"}
...
def predict_turnout(
    ...
    PANDSE_FACTORS = [
        ...
        _auto_druking_residual(),                          # 자동
        _auto_incumbent_premium(),                         # 자동
        {"name": "경남 보수 기저 회귀", "value": -1.0, "confidence": "high",
         "reason": "7대도 52.8% 천장. 숨은 보수 투표소 회귀"},  # 고정
