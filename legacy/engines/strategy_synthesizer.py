...
class DailyStrategy:
    ...
    # v3: attribution (행동-반응 귀인)
    attribution_summary: dict = field(default_factory=dict)  # AttributionSummary를 dict로 직렬화
...
class StrategySynthesizer:
    ...
    def synthesize(
        ...
        mode_override=None,              # v2: ModeDecision from strategy_mode_v2
    ) -> DailyStrategy:
        """
        핵심 메서드: 모든 입력을 종합하여 DailyStrategy를 생성.
        v2: mode_override가 주어지면 _determine_campaign_mode()를 스킵합니다.
        """
        ...
        # 1. 캠페인 모드 결정 — v2: mode_override 우선
        if mode_override and hasattr(mode_override, 'mode'):
            # v2 strategy_mode_v2.ModeDecision → CampaignMode 매핑
            _mode_map = {
                "ATTACK": CampaignMode.ATTACK,
                "DEFENSE": CampaignMode.DEFENSE,
                "INITIATIVE": CampaignMode.INITIATIVE,
                "CRISIS": CampaignMode.CRISIS,
            }
            mode = _mode_map.get(mode_override.mode, CampaignMode.INITIATIVE)
            mode_reasoning = f"[v2 모드 결정] {mode_override.reasoning} (신뢰도: {mode_override.confidence})"
        else:
