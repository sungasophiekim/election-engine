...
class IssueResponseEngine:
    ...
    def analyze_issue(
        ...
        readiness=None,  # v2: ReadinessScore
    ) -> IssueResponse:
        """
        ...
        Parameters:
            ...
            readiness: v2 ReadinessScore — 대응 준비도 (stance 결정에 영향)
        """
        ...
        # 3. 입장(stance) 결정 — v2: readiness 반영 + v3: learning feedback
        stance, stance_reason = self._determine_stance(
            keyword, score, level, impact, category, readiness=readiness
        )

        # v3: 학습 피드백으로 stance confidence 보정
        try:
            from engines.learning_feedback import adjust_confidence, get_cached_profile
            profile = get_cached_profile()
            if profile and profile.total_evaluated >= 5:
                sa = profile.stance_accuracy.get(stance)
                if sa and sa.total >= 5 and sa.accuracy_rate < 0.35:
                    # 이 stance의 정확도가 매우 낮으면 stance_reason에 경고 추가
                    stance_reason += f" ⚠️ 과거 정확도 {sa.accuracy_rate:.0%} — 신중 검토 필요"
        except Exception:
            pass
    ...
    def analyze_all(
        ...
        readiness_map: dict = None,  # v2: {keyword: ReadinessScore}
    ) -> list[IssueResponse]:
        ...
        for score in issue_scores:
            ...
            rdns = readiness_map.get(score.keyword)
            # DB에서 과거 트렌드 조회 시도
            historical = None
            try:
            ...
            resp = self.analyze_issue(score, sig, historical, readiness=rdns)
    ...
    def _determine_stance(
        self, keyword, score, level, impact, category, readiness=None
    ) -> tuple[str, str]:
        """
        5가지 stance 중 결정:
        ...
        v2: readiness (ReadinessScore) — 대응 준비도가 낮으면 stance 조정
        """
        # Crisis playbook 매칭
        ...
        # v2: readiness override — 준비도 점수가 stance 변경을 권고하면 적용
        if readiness and hasattr(readiness, 'recommended_stance_override'):
            override = readiness.recommended_stance_override
            if override and override != "":
                return (
                    override,
                    f"대응준비도 기반 stance 조정: {readiness.override_reason} "
                    f"(준비도 {readiness.total_readiness:.0f}%, 등급 {readiness.readiness_grade})",
                )
        ...
        if impact["danger"]:
            if level == CrisisLevel.CRISIS:
                ...
                # v2: 준비도 낮으면 counter → avoid 다운그레이드
                if readiness and hasattr(readiness, 'total_readiness') and readiness.total_readiness < 30:
                    return "avoid", f"고스코어({score:.0f}) + 부정적이나 대응준비도 부족({readiness.total_readiness:.0f}%) — 침묵 후 준비"
